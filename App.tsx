  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result && result.name) {
        ...
        alert("O servidor de nutrição não conseguiu processar este alimento agora. Tente novamente com um termo simples (ex: 'banana').");
      }
    } catch (e) { 
      alert("Houve uma falha na conexão. Verifique sua internet."); 
    }
    ...
    <div className="min-h-screen bg-[#fdfcf0] fade-in pb-12">
      <div className="max-w-5xl mx-auto p-4 md:p-8">
        <header className="mb-8 flex justify-between items-center bg-white p-5 rounded-3xl shadow-sm border border-slate-50">
          ...
          <button onClick={() => { if(confirm('Sair?')) { localStorage.clear(); window.location.reload(); } }} className="text-slate-200 hover:text-rose-400 p-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-5 space-y-6">
            {showTmbForm ? (
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                <h2 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6 text-center">Configurar Metas</h2>
                <form onSubmit={calculateTmb} className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5 text-left">
                      <label className="text-[11px] font-bold text-slate-700 uppercase ml-2 block">Peso (kg)</label>
                      <input type="number" step="0.1" value={weight} onChange={e => setWeight(Number(e.target.value))} className="bg-slate-50 p-4 rounded-2xl outline-none w-full font-bold focus:ring-2 focus:ring-rose-200" />
                    </div>
                    <div className="space-y-1.5 text-left">
                      <label className="text-[11px] font-bold text-slate-700 uppercase ml-2 block">Altura (cm)</label>
                      <input type="number" value={height} onChange={e => setHeight(Number(e.target.value))} className="bg-slate-50 p-4 rounded-2xl outline-none w-full font-bold focus:ring-2 focus:ring-rose-200" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5 text-left">
                      <label className="text-[11px] font-bold text-slate-700 uppercase ml-2 block">Idade</label>
                      <input type="number" value={age} onChange={e => setAge(Number(e.target.value))} className="bg-slate-50 p-4 rounded-2xl outline-none w-full font-bold focus:ring-2 focus:ring-rose-200" />
                    </div>
                    <div className="space-y-1.5 text-left">
                      <label className="text-[11px] font-bold text-slate-700 uppercase ml-2 block">Gênero</label>
                      <select value={gender} onChange={e => setGender(e.target.value as Gender)} className="bg-slate-50 p-4 rounded-2xl w-full font-bold cursor-pointer outline-none focus:ring-2 focus:ring-rose-200">
                  ...
                  <div className="space-y-1.5 text-left">
                    <label className="text-[11px] font-bold text-slate-700 uppercase ml-2 block">Nível de Atividade</label>
                    <select value={activity} onChange={e => setActivity(Number(e.target.value))} className="bg-slate-50 p-4 rounded-2xl w-full font-bold cursor-pointer outline-none focus:ring-2 focus:ring-rose-200">
                  ...
                  <button type="submit" className="w-full bg-rose-300 text-white font-black py-5 rounded-2xl shadow-lg hover:bg-rose-400 active:scale-95 transition-all">CALCULAR AGORA</button>
                </form>
              </div>
            ) : (
              <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-rose-50 space-y-4">
                <div className="flex justify-between items-center px-2">
                  ...
                  <button onClick={() => setShowTmbForm(true)} className="text-[10px] font-bold text-rose-300 uppercase underline">Alterar</button>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-rose-50 p-5 rounded-3xl text-center">
                    <p className="text-[9px] font-black text-rose-300 uppercase mb-1">TMB</p>
            ...
            <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-4">
              <h2 className="text-[11px] font-bold text-slate-700 uppercase tracking-widest px-2 text-left">Adicionar Alimento</h2>
              ...
              <div className="space-y-3">
                <input type="text" placeholder="Ex: Arroz integral cozido..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-sky-100" />
                <div className="flex gap-2">
                  <div className="relative flex-shrink-0">
                    <label className="absolute -top-2 left-3 bg-white px-1 text-[9px] font-black text-slate-400 uppercase">Gramas</label>
          ...
          <div className="lg:col-span-7 space-y-6">
                   ...
                   {[
                     ...
                     <div key={m.l} className="flex justify-between items-center bg-slate-50/70 p-4 rounded-2xl border border-white">
                       <div className="flex items-center gap-3">
                         ...
                         <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{m.l}</span>
                       </div>
            ...
            <div className="space-y-3 text-left">
              ...
              {foods.length === 0 && (
                <div className="bg-white/40 border-2 border-dashed border-slate-100 p-16 rounded-[3rem] text-center text-slate-200 text-xs font-bold uppercase tracking-widest">Nenhum registro hoje</div>
              )}
            </div>
          </div>
        </div>
